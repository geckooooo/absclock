...
func main() {
	...
	// Build the absolute time string using a string builder for efficiency
	...
	// Get the final absolute time string
	absTime := builder.String()*/

	// Also get the absolute time using the separate function for comparison
	currAbsoluteTime := getCurrentAbsoluteTime()

	fmt.Println(currAbsoluteTime)
}

// getAbsoluteTime builds the absolute time string by extracting the current time
